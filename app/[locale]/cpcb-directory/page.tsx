"use client";

import { useState } from "react";
import { useTranslations } from "next-intl";
import { ShieldCheck } from "lucide-react";
import { Card, CardHeader, CardTitle } from "@/components/ui/Card";
import { Button } from "@/components/ui/Button";
import { Link } from "@/i18n/navigation";
import { useAuth } from "@/lib/auth/AuthContext";
import { searchCpcbRecyclers } from "@/lib/api/cpcbRecyclers";
import { CpcbRecyclerSearchForm } from "@/components/cpcbdirectory/CpcbRecyclerSearchForm";
import { CpcbRecyclerResultCard } from "@/components/cpcbdirectory/CpcbRecyclerResultCard";
import type { CpcbRecyclerSearchResult } from "@/lib/api/types";

export default function CpcbDirectoryPage() {
  const t = useTranslations("cpcbDirectory");
  const { token } = useAuth();
  const [results, setResults] = useState<CpcbRecyclerSearchResult[] | null>(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [searched, setSearched] = useState(false);

  if (!token) {
    return (
      <div className="max-w-md mx-auto px-6 py-16 text-center">
        <div className="inline-flex rounded-full bg-[#0F6E56]/10 p-4 mb-6">
          <ShieldCheck className="h-10 w-10 text-[#0F6E56]" />
        </div>
        <h1 className="text-2xl font-bold text-[#444441] mb-3">{t("signInTitle")}</h1>
        <p className="text-[#444441]/70 mb-8 leading-relaxed">{t("signInBody")}</p>
        <div className="flex gap-3 justify-center">
          <Link href="/login"><Button variant="primary">{t("login")}</Button></Link>
          <Link href="/register"><Button variant="outline">{t("signupFree")}</Button></Link>
        </div>
      </div>
    );
  }

  async function handleSearch(params: { name?: string; gst?: string; stateId?: string }) {
    setLoading(true);
    setError(null);
    setSearched(true);
    try {
      const data = await searchCpcbRecyclers(params, token!);
      setResults(data);
    } catch (err) {
      setError(err instanceof Error ? err.message : t("searchFailed"));
      setResults(null);
    } finally {
      setLoading(false);
    }
  }

  return (
    <div className="max-w-3xl mx-auto px-6 py-10 space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-[#444441]">{t("title")}</h1>
        <p className="mt-1 text-sm text-[#444441]/60">{t("subtitle")}</p>
      </div>

      <Card>
        <CardHeader>
          <CardTitle>{t("cardTitle")}</CardTitle>
        </CardHeader>
        <CpcbRecyclerSearchForm onSearch={handleSearch} loading={loading} />
      </Card>

      {error && (
        <p className="text-sm text-[#A32D2D] bg-[#A32D2D]/5 border border-[#A32D2D]/20 rounded-md px-3 py-2">
          {error}
        </p>
      )}

      {searched && !loading && !error && (
        <div className="space-y-3">
          <p className="text-xs text-[#444441]/50">
            {t("resultCount", { count: results?.length ?? 0 })}
          </p>
          {results?.length === 0 ? (
            <Card>
              <p className="text-sm text-[#444441]/50 text-center py-6">{t("noResults")}</p>
            </Card>
          ) : (
            results?.map((r) => <CpcbRecyclerResultCard key={r.id} result={r} />)
          )}
        </div>
      )}
    </div>
  );
}
